...
public class OrderServiceImpl implements OrderService {
	...
	public OrderPojo processMatchEngine(OrderPojo orderPojo) throws Exception {
		...
		try {
			if (cacheDistributeLock.lock(DistributedLockKey.ORDER_BOOK_CACHE.name(), 200)) {
				isLocked = true;
				boolean orderExistInQueue = false;
				ConcurrentHashMap<String, TreeSet<OrderPojo>> currentQueue = OrderType.BUY
						.equals(orderPojo.getOrderType()) ? dataStorage.BUY_ORDER_BOOK_CACHE
								: dataStorage.SELL_ORDER_BOOK_CACHE;
				ConcurrentHashMap<String, TreeSet<OrderPojo>> targetQueue = OrderType.BUY
					...
					if (null != orderPojo.getPriceKey()) {
						...
						if (currentCacheOrder.getId() == orderPojo.getId()) {
							...
							LOGGER.info(String.format(
									"Start match, Price: %s, Quantity: %d, Price Type: %s, Target type: %s",
									targetPrice, orderPojo.getQuantity(), orderPojo.getPriceType(),
									orderPojo.getOrderType()));
							...
							while (currentCacheOrder.getQuantity() > 0L && null != targetPriceQueue
									&& targetPriceQueue.size() > 0) {
								hasMatched = true;
								OrderPojo targetOrder = targetPriceQueue.pollFirst();
								LocalDateTime currentTime = LocalDateTime.now();
								if (currentCacheOrder.getQuantity() - targetOrder.getQuantity() >= 0L) {
									currentCacheOrder
											.setQuantity(currentCacheOrder.getQuantity() - targetOrder.getQuantity());
									currentCacheOrder
											.setOrderStatus(currentCacheOrder.getQuantity() == 0 ? OrderStatus.FILLED
													: OrderStatus.PROCESSING);
									currentCacheOrder.setUpdateTime(currentTime);
									currentCacheOrder.getMatchOrderDetailPojo()
											.add(new OrderDetailPojo(targetOrder.getId(), currentTime,
													targetOrder.getPrice(), targetOrder.getQuantity()));

									targetOrder.setOrderStatus(OrderStatus.FILLED);
									targetOrder.setUpdateTime(currentTime);
									targetOrder.getMatchOrderDetailPojo()
											.add(new OrderDetailPojo(currentCacheOrder.getId(), currentTime,
													currentCacheOrder.getPrice(), targetOrder.getQuantity()));
									targetOrder.setQuantity(0L);

									dataStorage.FILLED_TASK_QUEUE.add(targetOrder);
								} else {
									targetOrder
											.setQuantity(targetOrder.getQuantity() - currentCacheOrder.getQuantity());
									targetOrder.getMatchOrderDetailPojo()
											.add(new OrderDetailPojo(currentCacheOrder.getId(), currentTime,
													currentCacheOrder.getPrice(), currentCacheOrder.getQuantity()));

									currentCacheOrder.setOrderStatus(OrderStatus.FILLED);
									currentCacheOrder.getMatchOrderDetailPojo()
											.add(new OrderDetailPojo(targetOrder.getId(), currentTime,
													targetOrder.getPrice(), currentCacheOrder.getQuantity()));
							...
							if (OrderStatus.PROCESSING.equals(currentCacheOrder.getOrderStatus())
									&& PriceType.LIMIT.equals(currentCacheOrder.getPriceType())) {
							...
							dataStorage.BUY_ORDER_BOOK_CACHE = OrderType.BUY.equals(currentCacheOrder.getOrderType())
									? currentQueue
									: targetQueue;
							dataStorage.SELL_ORDER_BOOK_CACHE = OrderType.BUY.equals(currentCacheOrder.getOrderType())
									? targetQueue
									: currentQueue;
